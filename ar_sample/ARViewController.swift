            ...
            if let camera = sceneView.pointOfView {
                ...
                for_i: for i in stride(from: -2, to: 2, by: 0.0005){
                    ...
                    self.sceneView.scene.rootNode.enumerateChildNodes {(node, _) in
                        ...
                        let isInRangeX: Bool = isInRange(tapPosition: position.x, objPosition: node.position.x, radius: 0.06)
                        let isInRangeY: Bool = isInRange(tapPosition: position.y, objPosition: node.position.y, radius: 0.06)
                        let isInRangeZ: Bool = isInRange(tapPosition: position.z, objPosition: node.position.z, radius: 0.06)
